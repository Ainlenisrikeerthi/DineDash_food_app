...
const StoreContextProvider = (props) => {
  ...
  const [deliveryCharge, setDeliveryCharge] = useState(() => {
    const savedState = localStorage.getItem("storeState")
    return savedState ? JSON.parse(savedState).deliveryCharge : 50 // Default delivery charge in rupees
  })
  ...
  const clearCart = () => {
    setCartItems({})
    setDeliveryCharge(50) // Reset to default delivery charge in rupees
    persistStoreState({}, 50) // Reset both cart and delivery charge
  }
  ...
  const contextValue = {
    ...
    applyPromoCode,
    clearCart,
    currency, // Added currency to context
  }
